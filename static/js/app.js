...
collectBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (resp.status === "success") {
      ...
      const m = resp.meta;
      const sourceCards = (m.sources || [])
        .map((s) => {
          const statusPill =
            s.status === "ok"
              ? '<span class="pill pill-high">collected</span>'
              : s.status === "empty"
                ? '<span class="pill pill-medium">0 matches</span>'
                : s.status === "skipped"
                  ? '<span class="pill pill-outline">skipped</span>'
                  : '<span class="pill pill-negative">error</span>';
          return `
          <div class="surface-card kpi-card">
            <p class="label">${escapeHtml(s.name)}</p>
            <p class="value" style="font-size:1.4rem;">${(s.matched_count || 0).toLocaleString()}</p>
            <div style="margin-top:0.5rem;">${statusPill}</div>
          </div>`;
        })
        .join("");
      ...
      summaryGrid.innerHTML =
        sourceCards ||
        `<div class="surface-card kpi-card"><p class="label">Total matched</p><p class="value">${(m.total_matched || 0).toLocaleString()}</p></div>`;
      ...
      if (m.sources_unavailable && m.sources_unavailable.length) {
        showToast(
          `Skipped/unavailable: ${m.sources_unavailable.join(", ")}. See setup notes for API keys.`,
          "error"
        );
      }
      setInlineStatus("success", `Collected ${(m.total_matched || 0).toLocaleString()} items across ${(m.sources || []).length} sources. Click \u201cRun Analysis\u201d to continue.`);
    } else {
...
function renderResults(data, meta) {
  document.getElementById("summaryHeadline").textContent =
    `${data.themes.length} themes, ${data.questions.length} research questions grounded across ${meta.total_matched || 0} items from multiple sources.`;
