...
Output JSON:
{
  ...
  // If path starts with /product-images/, resolve to public/product-images/ or images/
  if (url.pathname.startsWith('/product-images/')) {
    const pubPath = path.join(__dirname, 'public', url.pathname);
    const legacyPath = path.join(__dirname, 'images', path.basename(url.pathname).toLowerCase());
...
server.listen(PORT, () => {
  console.log(`Nykaa DECIDE MVP Server running on http://localhost:${PORT}`);
